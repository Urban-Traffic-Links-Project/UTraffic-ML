# Auto-converted from: 06_branchA_run_xt_forecast_prodcc_standalone_fast_per_lag(3).ipynb
...
def dmfm_predict(model: Dict[str, np.ndarray], R_origin: np.ndarray, lag: int) -> np.ndarray:
    ...
    # Fast path: cache A^lag once per horizon instead of recomputing for every sample.
    A_pows = model.get("A_pows", None) if isinstance(model, dict) else None
    if A_pows is not None and int(lag) in A_pows:
        A_pow = A_pows[int(lag)]
    else:
        A_pow = np.linalg.matrix_power(model["A"], int(lag))
...
def run_xt_direct_forecast(method_name: str, common_dir: Path, out_dir: Path, use_rt: bool,
    ...
    """Fast version: save ONLY per-lag metrics.

    Removed for speed:
    - no sample-level detail_metrics.csv
    - no compressed prediction .npz files
    - no coefficient summary/model .npz
    - no summary json

    Kept:
    - train one direct MultiTaskElasticNet model per horizon
    - evaluate val/test
    - save {method_name}_xt_per_lag_metrics.csv only
    """
    out_dir.mkdir(parents=True, exist_ok=True)
    ...
    print(f"[INFO] method={method_name}, use_rt={use_rt}, n_segments={n_segments}")
    print("[INFO] saving only per_lag_metrics.csv")

    # Cache DMFM model once. Also cache A^h for all horizons to avoid repeated matrix_power calls.
    dmfm_model = None
    if method_name == "dmfm":
        print("[INFO] fitting DMFM model once on train R_series ...")
        dmfm_model = fit_pca_factor_model(
            np.asarray(train["R_series"], dtype=np.float32),
            max_factors=DEFAULT_MAX_FACTORS,
        )
        dmfm_model["A_pows"] = {
            int(h): np.linalg.matrix_power(dmfm_model["A"], int(h)).astype(np.float32)
            for h in FORECAST_HORIZONS
        }
        print("[INFO] cached DMFM A powers:", sorted(dmfm_model["A_pows"].keys()))

    per_lag_rows = []

    for lag in FORECAST_HORIZONS:
        print(f"\n[HORIZON] lag={lag}: building train set ...")
        X_train, Y_train, _ = _build_dataset_for_horizon(
            ...
            use_rt=use_rt,
            shrink=dcc_shrink,
            dmfm_model=dmfm_model,
        )
        if len(X_train) == 0:
            print(f"[WARN] lag={lag}: empty train set, skipped")
            continue

        print(f"[HORIZON] lag={lag}: fit model X={X_train.shape}, Y={Y_train.shape}")
        model = fit_direct_model(X_train, Y_train, alpha=alpha, l1_ratio=l1_ratio)

        for split_name, data in [("val", val), ("test", test)]:
            print(f"[EVAL] lag={lag}, split={split_name}: building eval set ...")
            X_split, Y_split, _ = _build_dataset_for_horizon(
                method_name=method_name,
                train_data=train,
                split_data=data,
                lag=lag,
            ...
            if len(X_split) == 0:
                print(f"[WARN] lag={lag}, split={split_name}: empty eval set")
                continue
            Y_pred = model.predict(X_split).astype(np.float32)

            # Aggregate metric directly over all origins and all segments for this horizon.
            m = vector_metrics(Y_split, Y_pred)
            row = {
                "method": method_name,
                "split": split_name,
                "lag": int(lag),
            ...
            per_lag_rows.append(row)
            print(f"[METRIC] {method_name} | {split_name} | lag={lag} | MAE={m['mae']:.6f} | RMSE={m['rmse']:.6f}")

    per_lag = pd.DataFrame(per_lag_rows)
