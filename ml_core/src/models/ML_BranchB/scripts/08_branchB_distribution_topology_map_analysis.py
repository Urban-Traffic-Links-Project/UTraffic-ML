...
def plot_metric_by_lag(summary_df: pd.DataFrame, out_dir: Path, metric: str, split: str, methods: List[str]):
    ...
    for method in method_order(list(sub["method"].unique()), methods):
        ...
        if not g.empty:
            plt.plot(g["lag"], g[metric], marker="o", linewidth=2, label=METHOD_LABELS.get(method, method))
    plt.title(f"Branch B sampled directed G metric: {metric.upper()} by lag | {split.upper()}")
    plt.xlabel("Lag")
    plt.ylabel(metric.upper())
    plt.grid(True, alpha=0.3)
    plt.legend(fontsize=8, ncol=2)
    plt.tight_layout()
    path = out_dir / f"branchB_dist_metric_{metric}_{split}.png"
...
def plot_distribution_overlay(values_df: pd.DataFrame, out_dir: Path, split: str, lag: int, method: str, max_points: int = 200000):
    ...
    plt.hist(sub["true_value"], bins=80, alpha=0.55, density=True, label="G_true")
    plt.hist(sub["pred_value"], bins=80, alpha=0.55, density=True, label=f"G_pred {METHOD_LABELS.get(method, method)}")
    plt.title(f"Branch B distribution: G_true vs G_pred | {METHOD_LABELS.get(method, method)} | {split.upper()} | lag={lag}")
    plt.xlabel("Directed relation weight")
    plt.ylabel("Density")
    plt.grid(True, alpha=0.25)
    plt.legend()
    plt.tight_layout()
    path = out_dir / f"branchB_dist_overlay_{method}_{split}_lag{lag}.png"
    plt.savefig(path, dpi=180)
    plt.close()
    print("Saved:", path)


def plot_topology_metric(topology_df: pd.DataFrame, out_dir: Path, split: str, lag: int, metric: str, methods: List[str]):
    sub = topology_df[(topology_df["split"] == split) & (topology_df["lag"] == lag) & (topology_df["group_type"] == "hop_bin")].copy()
    if sub.empty or metric not in sub.columns:
        return
    ...
    for method in method_order(list(sub["method"].unique()), methods):
        g = sub[sub["method"] == method].sort_values("hop_bin")
        if not g.empty:
            plt.plot(g["hop_bin"].astype(str), g[metric], marker="o", linewidth=2, label=METHOD_LABELS.get(method, method))
    plt.title(f"Branch B OSM topology: {metric.upper()} by hop distance | {split.upper()} | lag={lag}")
    plt.xlabel("OSM line-graph hop distance")
    plt.ylabel(metric.upper())
    plt.grid(True, alpha=0.3)
    plt.legend(fontsize=8, ncol=2)
    plt.tight_layout()
    path = out_dir / f"branchB_topology_{metric}_{split}_lag{lag}.png"
    plt.savefig(path, dpi=180)
    plt.close()
    print("Saved:", path)


def make_error_by_edge_map(edge_meta, edge_error, out_path):
    if folium is None:
    ...
    if not required.issubset(edge_meta.columns):
        print("[WARN] metadata lacks coordinates; skip error map.")
        return
    meta = edge_meta.sort_values("position").reset_index(drop=True).copy()
    meta["edge_mean_abs_error"] = edge_error[:len(meta)]
    ...
    for _, row in meta.iterrows():
        err = float(row["edge_mean_abs_error"])
        extra = f"<b>mean abs directed G error:</b> {err:.6f}<br>"
        add_edge_polyline(fmap, row, color=color_err(err), weight=4 if err >= q80 else 3, opacity=0.82,
...
def main():
    ...
    default_out = branch_root / "results" / "08_distribution_topology_map_analysis"
    ...
    ap.add_argument("--error-map-method", type=str, default="factorized_tvpvar_gt")
    ...
    print_stage("BRANCH B — DISTRIBUTION / TOPOLOGY / ERROR MAP ANALYSIS")
    ...
    pair_i, pair_j = sample_directed_offdiag_pairs(N, args.pair_samples, rng)
    ...
    print_stage("SAMPLED DIRECTED G METRICS + TOPOLOGY")
    for split in args.splits:
        ...
        for lag in args.lags:
            ...
            for sample_id, (origin_idx, target_idx) in enumerate(pairs):
                ...
                true_vals = G_true[pair_i, pair_j].astype(np.float32)
                for method in args.methods:
                    t0 = time.time()
                    G_pred = predict_G(method, train, split_data, origin_idx, target_idx, lag, models, args)
                    pred_vals = G_pred[pair_i, pair_j].astype(np.float32)
                    ...
                    for _, r in by_hop.iterrows():
                        topology_rows.append({
                            "method": method, "split": split, "lag": int(lag), "sample_id": int(sample_id),
                            "group_type": "hop_bin", "hop_bin": r["hop_bin"], "geo_bin": "",
                            "n_pairs": int(r["n_pairs"]), "mae": float(r["mae"]), "rmse": float(r["rmse"]),
                        })
                    ...
                    for _, r in by_geo.iterrows():
                        topology_rows.append({
                            "method": method, "split": split, "lag": int(lag), "sample_id": int(sample_id),
                            "group_type": "geo_bin", "hop_bin": "", "geo_bin": r["geo_bin"],
                            "n_pairs": int(r["n_pairs"]), "mae": float(r["mae"]), "rmse": float(r["rmse"]),
                        })
                    print(f"  method={method:22s} sample={sample_id+1}/{len(pairs)} mae={m.get('mae', np.nan):.5f} rmse={m.get('rmse', np.nan):.5f}")

    dist_df = pd.DataFrame(dist_rows)
    topology_df = pd.DataFrame(topology_rows)
    dist_summary = dist_df.groupby(["method", "split", "lag"], as_index=False).mean(numeric_only=True)
    topology_summary = topology_df.groupby(["method", "split", "lag", "group_type", "hop_bin", "geo_bin"], as_index=False).mean(numeric_only=True)

    dist_df.to_csv(tables_dir / "branchB_distribution_metrics_by_snapshot.csv", index=False)
    dist_summary.to_csv(tables_dir / "branchB_distribution_metrics_summary.csv", index=False)
    topology_df.to_csv(tables_dir / "branchB_topology_error_by_snapshot.csv", index=False)
    topology_summary.to_csv(tables_dir / "branchB_topology_error_summary.csv", index=False)

    if value_blocks:
        values_df = pd.concat(value_blocks, ignore_index=True)
        values_df.to_csv(tables_dir / "branchB_sampled_true_pred_values.csv.gz", index=False, compression="gzip")
        for split in args.splits:
            for lag in args.lags:
                for method in args.methods:
                    plot_distribution_overlay(values_df, plots_dir, split, lag, method)
    for split in args.splits:
        for metric in ["mae", "rmse", "wasserstein", "ks_stat", "pearson"]:
            plot_metric_by_lag(dist_summary, plots_dir, metric, split, args.methods)
        for lag in args.lags:
            plot_topology_metric(topology_summary, plots_dir, split, lag, "mae", args.methods)
            plot_topology_metric(topology_summary, plots_dir, split, lag, "rmse", args.methods)

    print_stage("ERROR BY OSM EDGE MAP")
    if args.error_map_method in args.methods and args.map_split in all_data:
        map_data = all_data[args.map_split]
        map_pairs = sample_eval_pairs(map_data["meta"], args.map_lag, args.map_snapshots, rng)
        if map_pairs:
            edge_err_acc = np.zeros(N, dtype=np.float64)
            count = 0
            for pair_id, (origin_idx, target_idx) in enumerate(map_pairs, start=1):
                print(f"[MAP ERROR] pair {pair_id}/{len(map_pairs)}: origin={origin_idx}, target={target_idx}")
                G_true = get_G(map_data, target_idx)
                G_pred = predict_G(args.error_map_method, train, map_data, origin_idx, target_idx, args.map_lag, models, args)
                # directed row error: how well each source edge's outgoing relation vector is predicted
                edge_err_acc += np.mean(np.abs(G_pred - G_true), axis=1)
                count += 1
            edge_error = (edge_err_acc / max(count, 1)).astype(np.float32)
            map_path = maps_dir / f"branchB_{args.error_map_method}_error_by_osm_edge_map_{args.map_split}_lag{args.map_lag}.html"
            make_error_by_edge_map(edge_meta, edge_error, map_path)
