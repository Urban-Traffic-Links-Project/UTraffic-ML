"""
Branch B method module: No-Graph baseline.

No graph is used. 06B will set use_gt=False, so the velocity model is:
    x_hat[t+h] = A_h x_t

The pair generation is still fair with Granger-Gt methods: origins are allowed
only after the Granger history warm-up rows, if can_predict_granger exists.
"""
...
def check_branchB_common_dir_ready(common_dir: Path) -> None:
    ...
    for split in ["train", "val", "test"]:
        d = common_dir / split
        for name in ["z.npy", "segment_ids.npy", "timestamps.npy", "G_series_meta.csv"]:
            required.append(d / name)
    missing = [p for p in required if not p.exists()]
    if missing:
        raise FileNotFoundError("Missing No-Graph prepared files:\n" + "\n".join(map(str, missing)))
...
def load_gt_split(common_dir: Path, split: str) -> Dict[str, Any]:
    ...
    m = int(min(len(meta), z.shape[0], len(timestamps_raw)))
    if m <= 0:
    ...
    if "session_id" not in meta.columns:
        meta["session_id"] = meta["date_key"].astype(str) if "date_key" in meta.columns else "session_0"
    return {
        "z": z[:m],
...
def predict_G_method(*args, **kwargs):
    raise RuntimeError("No-Graph baseline should not call predict_G_method because use_gt=False.")
