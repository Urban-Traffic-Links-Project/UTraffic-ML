"""
Branch B method module: Persistence-Gt.
Uses current Granger-Gt at origin t as forecast for future graph.
"""
...
def predict_G_method(method_name: str, g_model: Dict[str, Any], split_name: str, split_data: Dict[str, Any], origin_idx: int, target_idx: int, horizon: int) -> np.ndarray:
    ...
    return np.asarray(G[_safe_idx(origin_idx, G.shape[0])], dtype=np.float32)
