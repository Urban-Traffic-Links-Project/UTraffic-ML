# ml_core/src/models/ML_BranchB/scripts/06D_true_rt_complex_formula.py
"""
Branch B — True-Rt XT forecast using the COMPLEX formula only.

Complex formula tested:
    z_hat[t+h] =
        sum_{r=0}^{p-1} A_{h,r} z[t-r]
        + sum_{ell=0}^{L_h-1} B_{h,ell}( W_true^{(ell)}[t->t+h] z[t-ell] )

In this implementation:
- z is the prepared normalized velocity tensor from Branch-B data.
- True-Rt is loaded from G_weight_series at target_idx = t+h.
- G_best_lag_series is used to approximate W_true^{(ell)}:
      W_true^{(ell)} keeps edges whose best_lag == h + ell.
- Then W is Top-K filtered, row-L1-normalized, and multiplied by z[t-ell].
- Features are:
      [z_t, z_{t-1}, ..., z_{t-p+1}, gx_0, gx_1, ..., gx_{L_h-1}]
...
Example:
    python ml_core/src/models/ML_BranchB/scripts/06D_true_rt_complex_formula.py \
      --topk 50 --lags 1-9 --p 4 --max-graph-lags 4 --max-nodes 512 --overwrite

Full:
    python ml_core/src/models/ML_BranchB/scripts/06D_true_rt_complex_formula.py \
      --topk 50 --lags 1-9 --p 4 --max-graph-lags 4 --overwrite
"""
...
def iter_eval_pairs_with_history(meta: pd.DataFrame, horizon: int, min_history: int) -> List[Tuple[int, int]]:
    """
    Return origin_idx, target_idx pairs without crossing sessions/days and with
    enough history before origin_idx.

    min_history=0 means z[t] only.
    min_history=3 means z[t], z[t-1], z[t-2], z[t-3] are available.
    """
    h = int(horizon)
    hist = int(min_history)
    if h <= 0:
...
def true_W_for_delay(
    ...
    """
    Build W_true for relation delay d=delay.

    lag_mode='matching':
        keep only edges where G_best_lag == delay.
    lag_mode='all':
        use the full mixed-best-lag True-Rt matrix for every delay.
        This is less theoretically clean but can be used as a diagnostic.
    """
    ...
    if lag_mode == "matching":
        ...
        return np.where(L == int(delay), G, 0.0).astype(np.float32)
...
def graph_lags_for_horizon(horizon: int, max_graph_lags: int, max_relation_lag: int) -> List[int]:
    """
    Return ell values satisfying d = h + ell <= max_relation_lag.
    """
    h = int(horizon)
    L = int(max_graph_lags)
    D = int(max_relation_lag)
    if L <= 0:
        return []
    return [ell for ell in range(L) if h + ell <= D]
...
def build_dataset_for_horizon(
    ...
    ell_values = graph_lags_for_horizon(horizon, max_graph_lags, max_relation_lag)
    max_ell = max(ell_values) if ell_values else 0
    min_history = max(int(p) - 1, max_ell)

    meta = split_data["meta"]
    pairs = iter_eval_pairs_with_history(meta, int(horizon), min_history=min_history)
    ...
    for origin_idx, target_idx in pairs:
        ...
        # Self/history velocity terms: z[t], z[t-1], ..., z[t-p+1]
        for r in range(int(p)):
            parts.append(z[origin_idx - r].astype(np.float32))

        # Graph terms: W^{ell}_{t->t+h} z[t-ell], with delay d=h+ell.
        for ell in ell_values:
            delay = int(horizon) + int(ell)
            x_lag = z[origin_idx - ell]
            W = true_W_for_delay(split_data, target_idx, delay=delay, lag_mode=lag_mode)
            gx = topk_graph_signal(W, x_lag, topk=topk, normalize=normalize, remove_self_loop=remove_self_loop)
            parts.append(gx.astype(np.float32))

        X_rows.append(np.concatenate(parts, axis=0).astype(np.float32))
        Y_rows.append(z[target_idx].astype(np.float32))

    n_parts = int(p) + len(ell_values)
    if not X_rows:
        return (
            np.empty((0, n_parts * N), dtype=np.float32),
            np.empty((0, N), dtype=np.float32),
            len(ell_values),
        )

    return np.vstack(X_rows).astype(np.float32), np.vstack(Y_rows).astype(np.float32), len(ell_values)
...
def run_horizon(
    ...
    ell_values = graph_lags_for_horizon(horizon, max_graph_lags, max_relation_lag)
    log(f"[h={horizon}] graph ell values={ell_values} -> delays={[horizon+e for e in ell_values]}")

    log(f"[h={horizon}] build train dataset")
    X_train, Y_train, n_graph_lags = build_dataset_for_horizon(
        train,
        horizon,
        p,
        max_graph_lags,
    ...
    for split_name, split_data in [("val", val), ("test", test)]:
        X_split, Y_split, _ = build_dataset_for_horizon(
            split_data,
            horizon,
            p,
            max_graph_lags,
        ...
        row = {
            ...
            "p": int(p),
            "max_graph_lags": int(max_graph_lags),
            "n_graph_lags_used": int(n_graph_lags),
            "max_relation_lag": int(max_relation_lag),
            **metrics,
        }
...
def build_arg_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument("--p", type=int, default=4, help="Number of self/history velocity lags: z[t]...z[t-p+1].")
    parser.add_argument("--max-graph-lags", type=int, default=4, help="Maximum number of graph lag terms ell.")
    parser.add_argument("--max-relation-lag", type=int, default=9, help="Maximum best_lag available in Branch-B GT.")
...
def main() -> None:
    ...
    if int(args.max_graph_lags) < 0:
        raise ValueError("--max-graph-lags must be non-negative")
    ...
    method_name = (
        f"formula_complex_true_rt_{args.lag_mode}_p{int(args.p)}_L{int(args.max_graph_lags)}_"
        f"topk{int(args.topk)}_{norm_tag}_{self_tag}_{intercept_tag}_{node_tag}"
    )
    ...
    for h in horizons:
        rows.extend(
            run_horizon(
                ...
                p=int(args.p),
                max_graph_lags=int(args.max_graph_lags),
                max_relation_lag=int(args.max_relation_lag),
                topk=int(args.topk),
                normalize=normalize,
    ...
    save_json(
        {
            "method_name": method_name,
            "formula": "z_hat[t+h] = sum_r A_hr z[t-r] + sum_ell B_hl(TopK(W_true_delay=h+ell) z[t-ell])",
            "p": int(args.p),
            "max_graph_lags": int(args.max_graph_lags),
            "max_relation_lag": int(args.max_relation_lag),
